...
Users who prefer to install FreeBSD using a graphical installer may be interested in https://www.furybsd.org[FuryBSD], https://ghostbsd.org[GhostBSD] or https://www.midnightbsd.org[MidnightBSD].
====
...
* `Boot Single User`: This mode can be used to fix an existing FreeBSD installation as described in <<boot/#boot-singleuser, Single-User Mode>>. Press kbd:[2] or the upper- or lower-case kbd:[S] to enter this mode.
...
* `secure_console` - When this option is enabled, the prompt requests the `root` password when entering single.

...
Use the `-l` argument to man:ls[1] to view a long directory listing that includes a column of information about a file's permissions for the owner, group, and everyone else. For example, a `ls -l` in an arbitrary directory may show:
...
....
+
. Use man:kill[1] to send the signal. Because man:inetd[8] is owned by `root`, use man:su[1] to become `root` first.
+
[source,bash]
....
% su

...
Users who prefer an installation method that automatically configures the Xorg should refer to https://www.furybsd.org[FuryBSD], https://ghostbsd.org[GhostBSD] or https://www.midnightbsd.org[MidnightBSD].
====
...
Now Gimp, Apache OpenOffice, and all of the other X applications should now recognize the installed TrueType(R) fonts. Extremely small fonts (as with text in a high resolution display on a web page) and extremely large fonts (within StarOffice(TM)) will look much better now.

...
Subversion ist hauptsächlich ein Werkzeug für Entwickler. Die meisten Benutzer bevorzugen `freebsd-update` (<<updating-upgrading-freebsdupdate>>) um das FreeBSD Basissystem zu aktualisieren, und `portsnap` (<<ports-using>>) um die FreeBSD Ports-Sammlung aktuell zu halten.
====

...
Der Rest dieses Abschnitts beschreibt, wie FreeBSD für gebräuchliche PPP-Verbindungsszenarien konfiguriert wird. Die erforderliche Konfigurationsdatei ist [.filename]#/etc/ppp/ppp.conf#. Zusätzliche Dateien und Beispiele sind in [.filename]#/usr/share/examples/ppp/# verfügbar.
...
Um bestimmten Benutzern eine statische IP-Adresse zuzuweisen, können Sie die Adresse als drittes Argument in [.filename]#/etc/ppp/ppp.secret# angeben. Beispiele finden Sie in [.filename]#/usr/share/examples/ppp/ppp.secret.sample#.

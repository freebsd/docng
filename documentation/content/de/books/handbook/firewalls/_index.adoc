...
[.note]
====
[.admontitle]*Es ist möglicherweise nicht notwendig, alle aggressiven Rechner zu blockieren:* +
...
[.tip]
====
[.admontitle]*Wählen Sie die Datenquellen mit Sorgfalt:* +
...
Mit der hier beschriebenen Regel-Syntax können zustandsorientierte Regeln erstellt werden. Beim Erstellen von Regeln ist zu beachten, dass Regeln ohne das Schlüsselwort `quick` der Reihe nach geprüft werden und "die letzte zutreffende Regel" angewendet wird. Das bedeutet, dass selbst dann, wenn die erste zutreffende Regel eine `pass`-Regel ist, das Paket dennoch geblockt wird, falls später eine `block`-Regel zutrifft. Beispielregelsätze finden Sie in [.filename]#/usr/share/examples/ipfilter#.

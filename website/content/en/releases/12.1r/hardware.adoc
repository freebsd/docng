...
= FreeBSD {{% param releaseCurrent %}} Hardware Notes
...
This document contains the hardware compatibility notes for FreeBSD {{% param releaseCurrent %}}. It lists the hardware platforms supported by FreeBSD, as well as the various types of hardware devices (storage controllers, network interfaces, and so on), along with known working instances of these devices.
...
If you have a system that is not listed here, it may not have been tested with FreeBSD {{% param releaseCurrent %}}. We encourage you to try it and send a note to the A.SPARC with your results, including which devices work and which do not.
